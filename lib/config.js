...
async function getServerConfig() {
    try {
        ...
        // This ensures the bot "learns" the server on its first run without manual SQL input.
        const needsSync = !data.guild_id && process.env.DISCORD_GUILD_ID;
        if (needsSync) {
            ...
            const updates = {
                ...
                bot_id: data.bot_id || process.env.DISCORD_CLIENT_ID,
                admin_role_ids: data.admin_role_ids || process.env.DISCORD_ADMIN_ROLE_ID,
                role_accepted: data.role_accepted || process.env.DISCORD_ROLE_ID,
                role_certified: data.role_certified || process.env.DISCORD_CERTIFIED_ROLE_ID,
                role_alumni: data.role_alumni || process.env.DISCORD_ALUMNI_ROLE_ID,
                role_student: data.role_student || process.env.DISCORD_STUDENT_ROLE_ID,
                role_2030: data.role_2030 || process.env.DISCORD_ROLE_2030,
        ...
        return {
            ...
            emailFromAddress: data?.email_from_address || process.env.EMAIL_FROM_ADDRESS || 'Bot Verifies <verify@example.com>',
